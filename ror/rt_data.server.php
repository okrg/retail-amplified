...
$sql="SELECT projects.sitename,
	...
	AND rt_rors.loc_key= $id 
	AND rt_rors.status = '$status' 
	ORDER BY rt_rors.creation DESC";
...
$count_query = mysql_query("select id from rt_rors where loc_key = $id AND status='$status'");
$count = mysql_num_rows($count_query);

if (!$rx){
	$main = "<p><a href=\"#\" onClick=\"javascript:ajax_do('rt_data.php?s=$status&reset=1&loc=".$id."');return false;\"><img src=\"images/minus.gif\" border=\"0\" /></a>&nbsp;";
	$main .= "<a href=\"#\" onClick=\"javascript:ajax_do('rt_data.php?s=$status&reset=1&loc=".$id."');return false;\">".myTruncate($r['sitename'],30, " ")."</a> <span class=\"count\">($count)</span>";
	$main .= " <a href=\"javascript:workit(".$r['id'].",'workset');\"><img align=\"absmiddle\" border=\"0\" src=\"images/start.gif\" /><a/>";
	$main .= "<ul class=\"main\">";
} else {
	$main = "<p><a href=\"#\" onClick=\"javascript:ajax_do('rt_data.php?s=$status&loc=".$id."');return false;\"><img src=\"images/plus.gif\" border=\"0\" /></a>&nbsp;";
	$main .= "<a href=\"#\" onClick=\"javascript:ajax_do('rt_data.php?s=$status&loc=".$id."');return false;\">".myTruncate($r['sitename'],30, " ")."</a> <span class=\"count\">($count)</span>";
	$main .= " <a href=\"javascript:workit(".$r['id'].",'workset');\"><img align=\"absmiddle\"  border=\"0\" src=\"images/start.gif\" /><a/>";
	$main .= " </p>";
	$main .= "<ul class=\"main\">";
}
...
if (!$rx){
	...
	while ($row = mysql_fetch_object($result)){
		$sqly = "select * from rt_ror_responses where parent_key = ".$row->id."";
		...
		$main .= "<a class=\"items\" onclick=\"ViewPop('view.php?mode=$mode&id=$row->id','view');return false;\" href=\"#\">$body</a>&nbsp;$count</li>";	//Body (truncated)	
		...
		$date .= "<li class=\"$read date\">$row->creation</li>";						//Creation Date / Time
		}
}
$main .= "</ul>";
$city .= "</ul>";
...
if (!$rx){
	...
	while ($row = mysql_fetch_object($result)){
		$sqly = "select * from rt_freq_responses where parent_key = ".$row->id."";
		$resulty = mysql_query($sqly);
		if (!$resulty) {error("Error with database: ".mysql_error());exit;}
		$count = mysql_num_rows($resulty);
		if ($count > 0) {$count="<span class=\"msgcount\">$count</span>";}else {$count="";}
		...
		//$main .= "<input name=\"workbox[]\" type=\"checkbox\" value=\"".$row->id."\" />";
		$main .= "<a class=\"items\" onclick=\"ViewPop('view.php?mode=$mode&id=$row->id','view');return false;\" href=\"#\">$body</a>&nbsp;$count</li>";	//Body (truncated)	
		$city .= "<li class=\"$read city\">Qty: $row->qty </li>";
		$state .= "<li class=\"$read state\">".ucwords($row->order_status)."&nbsp;</li>";
		$store .= "<li class=\"$read store\">&nbsp;</li>";
		$dist .= "<li class=\"$read dist\">&nbsp;</li>";
		$reg .= "<li class=\"$read reg\">&nbsp;</li>";
		$urgency .= "<li class=\"$read urgency\"><img src=\"images/".$u[$row->urgency][1].".gif\" />&nbsp;".$u[$row->urgency][0]."</li>";				//Urgency
