<p>These are the budget documents for this project. Only the Charlotte Russe users can view these files.</p>
<?php
//Repeat process for budget folder
	//scan for folders and files INSIDE PHOTOS that are NOT thumbs and NOT system folders or files
	$filespace = "filespace/$id/budget/";
...
//Render containing table
	...
	foreach($files as $key=>$value)
	{
		...
		if ($usergroup == "0")
		{
		echo "<a href=\"#delete_budget\" onClick=\"javascript:toggleBox('delete_budget',1);setDelbudgFile('".$filespace."','".$value."');\"><img src=\"images/delete.gif\" border=\"0\" /></a>";
		}
		...
		echo "<td class=\"files\" align=\"left\">Budget Document</td>";
		...
		//Get comments and author from BUDGETLOG then  format and show them
		$sql = "select author, comment from budgetlog where filename='$value' and project= $id";
	...
	if ($usergroup == 0)
	{
		...
		echo "<a href=\"#budget\" onClick=\"javascript:toggleBox('budget',1);\">Upload a new budget file</a><br />";
	}
...
?>	
	<br />
	<div id="budget" class="filebox" style="display:none;">
	<a name="budget"></a>
	<p><a href="javascript:toggleBox('budget',0);"><small>(Hide)</small></a></p>
	<p>
	<table align="center" cellspacing="8" cellpadding="8">
	<tr>
	<td><img src="images/avatar_single.gif" /></td>
	<td>
	<h2>::Upload a single budget file</h2>
	<p>Add a <strong>single file</strong> to the budget folder. Only corp team and GDA will be able to see these files.</p>
	<form name="single" method="post" action="<?php echo "$PHP_SELF?page=project&mode=budget"; ?>" enctype="multipart/form-data" onSubmit="return validateSingle(this);">
		...
		<input class="files" type="reset" name="reset" value="cancel" onClick="javascript:toggleBox('budget',0);">
		</td>
		</tr>
		</table>
	</form>
	...
	<br />
	<div id="delete_budget" class="filebox" style="display:none;">
	...
	<form name="delbudg" method="post" action="<?php echo "$PHP_SELF?page=project&mode=del"; ?>" enctype="multipart/form-data" >
	<table width="99%" cellspacing="10" class="litezone">
		<tr>
		<td width="200" align="right" valign="top"><small><strong>Do you want to delete this file?</strong></small></td>
		<td>
		<input type="hidden" name="delbudg_file_path"></input>
		<input type="hidden" name="project_id" value="<?php echo $id; ?>"></input>
		<input class="files" type="text" name="delbudg_file_name" size="50"></input><br /><br />
		<input class="files" type="submit" name="submit" value="Yes, delete" onKeyPress="return noenter()">&nbsp;
		<input class="files" type="reset" name="reset" value="No, cancel" onClick="javascript:toggleBox('delete_budget',0);">
		</td>
		</tr>
		</table>
	</form>
