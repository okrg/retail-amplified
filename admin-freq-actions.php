...
if ($_GET['action'] == "close") {
	...
	if ($_POST['clearbox'] == "") {
		...
		$sql = "update fixture_orders set status='clear' where id = ".$value;
			if (!mysql_query($sql)) {
...
if ($_GET['action'] == "trash") {
	...
	if ($_POST['clearbox'] == "") {
		...
		foreach($_POST['clearbox'] as $value) {
		$sql = "delete from fixture_orders where id = ".$value;
			if (!mysql_query($sql)) {
				error("A database error occured: " . mysql_error());
			}
		$count++;
...
if (($_GET['action'] == "report") || ($_POST['action'] == "report")) {
	...
	if (isset($_POST['followup'])) {
		foreach($_POST['followup'] as $value) {
			$reporting_fu_sql .= "followup=".$value;
			$reporting_fu_sql .= " or ";
			if ($value==1) {
				$fr .= "<li>Follow-up <strong>required</strong></li>";
				} else {
				$fr .= "<li>Follow-up <strong>not required</strong></li>";
				}
		}
			$reporting_fu_sql .= "followup=2";
