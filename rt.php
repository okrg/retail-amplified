...
$season = "jun10";
$season_nice = "<span style=\"color:red;font-weight:bold;\">June</span>";
...
function Fixtures($cat) {
	...
	$result=mysql_query("select * from fixture_key where chain=1 and exclude=0 and cat = '$cat' order by name");
	} else { //show all then
	$result=mysql_query("select * from fixture_key where chain=1 and exclude=0 order by cat");
	}
...
//		}
	...
	$result = mysql_query($sql);
	if (!$result) {error("Error with database: $sql ".mysql_error());}
...
function mysql_do($sql) {
    $r = mysql_query($sql) or die("query:".$sql.mysql_error());
